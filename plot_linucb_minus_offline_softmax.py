...
def _read_main_rows(results_path: Path) -> list[dict[str, str]]:
    ...
    return [
        ...
        and row["policy"] in {"linucb_raw", "softmax_best_arm"}
    ]


def plot_linucb_minus_offline_softmax(packet_dir: Path) -> Path:
    ...
    for d in d_values:
        ...
        for (row_d, _seed_idx), by_policy in regrets.items():
            if row_d != d:
                continue
            if "linucb_raw" in by_policy and "softmax_best_arm" in by_policy:
                gaps.append(by_policy["linucb_raw"] - by_policy["softmax_best_arm"])
    ...
    ax.set_xlabel("d")
    ax.set_ylabel("LinUCB regret - offline softmax regret")
    ax.set_title("LinUCB vs offline supervised softmax gap")
    ax.legend(fontsize=8)
    fig.tight_layout()

    output_path = packet_dir / "plots" / "main" / "linucb_minus_offline_softmax_vs_d.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot LinUCB minus offline supervised softmax regret for one report packet."
    )
    parser.add_argument(
        "--packet-dir",
        type=Path,
    ...
    plot_linucb_minus_offline_softmax(args.packet_dir)
